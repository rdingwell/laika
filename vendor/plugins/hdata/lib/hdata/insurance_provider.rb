    ...
    module InsuranceProvider
      ...
      def to_hdata(xml)
        ...
        "xmlns:core" => Hdata::Core::NAMESPACE) do       
          xml.id group_number if group_number
          xml.healthInsuranceType("code"=>insurance_type.code, "codeSystem"=>insurance_type.code_system, "displayName"=>insurance_type.name)
          ...
          attrs ={}
          attrs[:low] =>start_service if start_service
          attrs[:high] => end_service if end_service
          xml.healthPlanCoverageDates(attrs) if !attrs.empty?
          
          xml.patient do
            Hdata::Core::PersonLike.to_hdata(insurance_provider_patient,xml)
            xml.memberId insurance_provider_patient.member_id if insurance_provider_patient.memeber_id
          ...
          xml.subscriber do
            ...
            xml.subscriberId(insurance_provider_subscriber.subscriber_id)
