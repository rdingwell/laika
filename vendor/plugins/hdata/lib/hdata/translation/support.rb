  ...
  module Translation
    module Support
      ...
      def to_hdata(xml)
        ...
        xml.support(:xmlns => "http://projecthdata.org/hdata/schemas/2009/06/support",
              "xmlns:core" => Hdata::Core::NAMESPACE) do
          #dont know which one to pick start or end, think the schema needs to be revisited
          if start_support
             xml.date(start_support.to_s(:xmlDate))
          elsif end_support
              xml.date(end_support.to_s(:xmlDate) )
          end
          xml.contactType(contact_type.code)
          #relationship codes are specified in a different code system and I'm not translating that!'
          # if relationship
          #   xml.contactRelationship
          # end
          xml.contact do
