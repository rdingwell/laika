    ...
    module AdvanceDirective
      ...
      def to_hdata(xml)
        xml.instruct! :xml, :version => '1.0'
          xml.advanceDirective(:xmlns => "http://projecthdata.org/hdata/schemas/2009/06/advance_directive",
            ...
            xml.freeTextType(free_text)
              ...
              attrs = {}
              attrs[:low] = start_effective_time.to_s(:iso8601) if start_effective_time.present?
              attrs[:high] = end_effective_time.to_s(:iso8601) if end_effective_time.present?          
              xml.effectiveDate(attrs) if !attrs.empty?
            

            xml.custodianOfTheDocument do
              xml.tag!("core:contact") do
       ...
       def to_atom(xml)
          xml.entry do
          	xml.title "Advance Dricetive"
         		xml.link(:href=>"/hdata/#{patient.id}/advance_directives/#{self.id}")
         		xml.id(self.id)
          	xml.summary("")
         	end
       end
