    ...
    module Immunization
      ...
      def to_hdata(xml)
        ...
        "xmlns:core" => Hdata::Core::NAMESPACE) do
          xml.refusal !no_immunization_reason.present?
          xml.administeredDate administration_date.to_s(:iso8601) if administration_date       
          xml.medicationInformation do
            xml.tag!("med:manufacturedMaterial") do 
              xml.tag!("med:codedProductName",'code' => vaccine.code, 
                       'codeSystem' => '2.16.840.1.113883.6.59',
                       'displayName' => vaccine.name) do
                         xml.tag!("med:freeTextProductName", vaccine.name)
                       end
            end
            
          end
          if no_immunization_reason
            xml.refusalReason('code' => no_immunization_reason.code, 'codeSystem' => '2.16.840.1.113883.6.96', 'displayName'=>no_immunization_reason.name)
          end
          xml.narrative
        end
