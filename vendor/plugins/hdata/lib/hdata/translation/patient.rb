    ...
    def to_hdata(xml)
      ...
      xml.patient(:xmlns => "http://projecthdata.org/hdata/schemas/2009/06/patient_information",
        ...
        if registration_information.present?
          Hdata::Core::PersonLike.to_hdata(self.registration_information,xml)
        else       
          xml.name do
            xml.given(name)
          end
        ...
        if registration_information.present?
         ...
         # do the gaurdian stuff here non gaurdian is placed elsewhere
          if support && support.contact_type.try(:code) == "GUARD"
             xml.guardian do
               Hdata::Core::PersonLike.to_hdata(support,xml)
             end
          end
        ...
        def to_atom(xml)
           xml.entry do
           	xml.title "#{self.person_name.first_name} #{self.person_name.last_name}"
          		xml.link(:href=>"/hdata/#{patient.id}/patient_information/#{self.id}")
          		xml.id(self.id)
          	  xml.updated self.updated_at.to_s(:iso8601)
           	xml.summary("#{self.person_name.first_name} #{self.person_name.last_name}")
          	end
        end
