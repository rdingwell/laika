module Hdata
  module Translation
    ...
    module AbstractResult
         ...
         def to_hdata(xml)
            ...
            xml.result(:xmlns => "http://projecthdata.org/hdata/schemas/2009/06/result",
                           "xmlns:core" => Hdata::Core::NAMESPACE,
                           "xmlns:xsi"=>"http://www.w3.org/2001/XMLSchema") do  
              xml.resultId(:root=>result_id)
              if result_date.present?
                xml.resultDateTime(:low=>result_date.to_s(:iso8601))
             end
              ...
              if act_status_code
                xml.resultStatus(:code=>act_status_code.code)
              end
       ...
       def to_atom(xml)
         xml.entry do
         	xml.title result_code_display_name
        			xml.link(:href=>"/hdata/#{patient.id}/#{self.class.name.to_s.underscore}s/#{self.id}")
        		xml.id(self.id)

         	xml.summary(result_code_display_name)
        	end
      end
    ...
    module VitalSign
      ...
      def to_hdata(xml)
        xml.instruct! :xml, :version => '1.0' 
          xml.vitalSigns(:xmlns => "http://projecthdata.org/hdata/schemas/2009/06/vital_signs",
                         "xmlns:core" => Hdata::Core::NAMESPACE) do    
          if result_date.present?
             xml.effectiveTime(result_date.to_s(:iso8601))
          end    
