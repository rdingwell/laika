  ...
  module Translation
    module Procedure
      ...
      def to_hdata(xml)
        ...
        xml.procedure(:xmlns => "http://projecthdata.org/hdata/schemas/2009/06/procedure",
                       "xmlns:core" => Hdata::Core::NAMESPACE) do      
         xml.procedureId("root"=>procedure_id)
         if self.code
          xml.procedureType("code" => self.code, "codeSystem" => "2.16.840.1.113883.6.96", :displayName=>name)
         end
         xml.procedureFreeTextType(name)
         if procedure_date
           xml.procedureDate(procedure_date.to_s(:iso8601))
         end
         xml.narrative
        end                     
      end
       ...
       def to_atom(xml)
         xml.entry do
         	xml.title name
        			xml.link(:href=>"/hdata/#{patient.id}/procedures/#{self.id}")
        		xml.id(self.id)
         	xml.summary(name)
        	end
      end
