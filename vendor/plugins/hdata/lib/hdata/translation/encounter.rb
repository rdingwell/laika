    ...
    module Encounter
      ...
      def to_hdata(xml)
        ...
        xml.encounter(:xmlns => "http://projecthdata.org/hdata/schemas/2009/06/encounter",
          ...
          xml.encounterId(:root=>encounter_id)   
          if encounter_type
            xml.encounterType("code" => encounter_type.code, "codeSystem" => "2.16.840.1.113883.5.4", "displayName" => encounter_type.name)
            xml.encounterFreeTextType( free_text)
          end
          if encounter_date.present?
            xml.encounterDate( :low=>encounter_date.to_s(:brief))
          end
          xml.narrative()       
        end                     
      end
