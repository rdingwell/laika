  ...
  module Translation
    module HealthcareProvider
      ...
      def to_hdata(xml)
        ...
        xml.provider(:xmlns => "http://projecthdata.org/hdata/schemas/2009/06/provider",
              "xmlns:core" => Hdata::Core::NAMESPACE) do
          attrs = {}
          attrs[:low] = start_service.to_s(:iso8601) if start_service.present?
          attrs[:high] = end_service.to_s(:iso8601) if end_service.present?          
          xml.dateRange(attrs) if !attrs.empty?
          if provider_role
            xml.providerRoleCoded("code" => provider_role.code,
                             "displayName" => provider_role.name,
                             "codeSystem" => "2.16.840.1.113883.12.443", 
                             "codeSystemName" => "Provider Role")
            xml.providerRoleFreeText provider_role_free_text
          end
          if provider_type
            xml.providerType("code" => provider_type.code,
                     "displayName" => provider_type.name.gsub("&","and"),
                     "codeSystem" => "2.16.840.1.113883.6.101",
                     "codeSystemName" => 'ProviderCodes')
          end
          xml.providerEntity do
            xml.tag!("core:person") do
              Hdata::Core::PersonLike.to_hdata(self,xml)
            end
          end
          xml.providerPatientId patient_identifier if patient_identifier
          xml.narrative
       end
      end
      ...
      def to_atom(xml)
         xml.entry do
         	xml.title "Healthcare Provider - #{self.person_name.first_name} #{self.person_name.last_name}"
        			xml.link(:href=>"/hdata/#{patient.id}/healthcare_providers/#{self.id}")
        		xml.id(self.id)

         	xml.summary("Healthcare Provider - #{self.person_name.first_name} #{self.person_name.last_name}")
        	end
      end
